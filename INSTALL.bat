...
echo.
echo Kreiranje pomocnog fajla za precicu...
echo @echo off > "%APP_DIR%\open_portal.bat"
echo start "" "http://localhost:8080" >> "%APP_DIR%\open_portal.bat"

echo Kreiranje Desktop precice...
powershell -Command "$ws = New-Object -ComObject WScript.Shell; $sc = $ws.CreateShortcut([Environment]::GetFolderPath('Desktop') + '\\Port Servisni Portal.lnk'); $sc.TargetPath = 'C:\\PortServis\\open_portal.bat'; $sc.WindowStyle = 7; $sc.IconLocation = 'C:\\PortServis\\portservis.ico'; $sc.Description = 'Port Servisni Portal'; $sc.Save()"
echo Desktop precica kreirana!
...
copy "%SRC%\portservis.ico" "%APP_DIR%\portservis.ico" >nul 2>&1
